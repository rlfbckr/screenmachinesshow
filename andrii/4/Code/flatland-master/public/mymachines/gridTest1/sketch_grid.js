...
class Machine extends defaultMachine {
    setup() {
        ...
        */
        ...
        this.myownrandomradius =  random(20, 150);
...
let index = 0;
let grid = [];
let maxpoints = 4; // wieviele punkte
    ...
    for (var y = 0; y < maxpoints; y++) { // für jede zeile
        for (var x = 0; x < maxpoints; x++) { // für jede spalte
            var v = createVector(
                ...
                map(y, 0, maxpoints -1, -(height / 2) + margin+random(50), (height / 2) - margin)
            );
            console.log(v.x + ' ' + v.y);
            grid.push(v);
        }
