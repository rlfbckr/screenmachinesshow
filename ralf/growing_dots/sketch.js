let balls = [];
let anzahl = 200; // wieviele bälle will ich erzeugen!!
let nid = 0;
let maxage = 100000;
let myFont;
let last_birth = 0;
let birth_interval = 1;
	...
	for (let j = 0; j < balls.length; j++) {
		for (let i = 0; i < balls.length; i++) {
			if (i != j) {
				...
				if ((millis()-balls[j].last_grow)>200 && dist(balls[j].xpos, balls[j].ypos, balls[i].xpos, balls[i].ypos) < 3) {
					//	noStroke();
					 ellipse(balls[j].xpos, balls[j].ypos, 10);
					balls[j].size = balls[j].size + 1; //  + balls[i].size)/2;
					//balls[i].delete = true;
					balls[j].last_grow = millis();
